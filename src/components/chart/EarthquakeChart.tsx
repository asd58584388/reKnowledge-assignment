...
const EarthquakeChart: React.FC<EarthquakeChartProps> = ({ data }) => {
  ...
  const chartData = useMemo(() => {
    return data
      .filter(earthquake => isEarthquakeVisible(earthquake))
      .map(earthquake => ({
        id: earthquake.id,
  ...
  }, [data, xAxis, yAxis, selectedId, hoveredId, isEarthquakeVisible]);
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white p-3 border border-gray-300 rounded shadow-lg max-w-xs">
          ...
          <p className="text-sm text-gray-600">Magnitude: {formatNumber(dataPoint.magnitude, 1)}</p>
          <p className="text-sm text-gray-600">{getAxisLabel(xAxis)}: {formatNumber(dataPoint.x)}</p>
          <p className="text-sm text-gray-600">{getAxisLabel(yAxis)}: {formatNumber(dataPoint.y)}</p>
          <p className="text-xs text-gray-400 mt-1">Click to select • Hover to highlight</p>
        </div>
      );
    }
    return null;
  ...
  const getAxisLabel = useCallback((axis: keyof ProcessedEarthquakeData): string => {
    const column = numericColumns.find(col => col.key === axis);
    return column?.label || String(axis);
  }, [numericColumns]);

  const getDotColor = useCallback((dataPoint: ChartDataPoint): string => {
  ...
  const getDotSize = useCallback((dataPoint: ChartDataPoint): number => {
    let baseSize = Math.max(4, Math.min(12, dataPoint.magnitude * 2));
  ...
  const getDotOpacity = useCallback((dataPoint: ChartDataPoint): number => {
  ...
  const handleDotClick = useCallback((dataPoint: ChartDataPoint) => {
    ...
    if (earthquake) {
      setSelectedId(dataPoint.id);
      setSelectedEarthquake(earthquake);
    }
  }, [data, setSelectedId, setSelectedEarthquake]);
  ...
  }, [getDotColor, getDotSize, getDotOpacity, handleDotHover, handleDotClick]);
      ...
      <div className="flex-shrink-0 p-4 bg-gray-50 border-b">
        ...
        <div className="flex gap-4">
          <div className="flex items-center gap-2">
            ...
            <select
              value={xAxis}
              onChange={(e) => setXAxis(e.target.value as keyof ProcessedEarthquakeData)}
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              value={yAxis}
              onChange={(e) => setYAxis(e.target.value as keyof ProcessedEarthquakeData)}
              className="px-3 py-1 border border-gray-300 rounded text-sm"
            >
              {numericColumns.map(col => (
                <option key={col.key} value={col.key}>{col.label}</option>
        ...
        <div className="mt-2 text-xs text-gray-500">
          💡 Click chart points or table rows for bi-directional selection • Hover for preview
        </div>
      </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <ScatterChart margin={{ top: 20, right: 20, bottom: 40, left: 40 }}>
            ...
            <XAxis 
              ...
              label={{ value: getAxisLabel(xAxis), position: 'insideBottom', offset: -10 }}
            />
            <YAxis 
              type="number" 
              dataKey="y" 
